...
#ifdef HAVE_PORTREG
#define GP1294_SELECT *csPort &= ~csPinMask;       ///< Device select
#define GP1294_DESELECT *csPort |= csPinMask;      ///< Device deselect
#else
#define GP1294_SELECT digitalWrite(csPin, LOW);       ///< Device select
#define GP1294_DESELECT digitalWrite(csPin, HIGH);    ///< Device deselect
#endif
...
/*!
    @brief  Constructor for SPI GP1294VFD displays, using software (bitbang)
            SPI.
    @param  w
    ...
    @return GP1294VFD object.
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
GP1294VFD::GP1294VFD(uint16_t w, uint8_t h, int8_t mosi_pin,
                                   int8_t sclk_pin, 
...
/*!
    @brief  Constructor for SPI GP1294VFD displays, using native hardware SPI.
    @param  w
            Display width in pixels
    @param  h
            Display height in pixels
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
GP1294VFD::GP1294VFD(uint16_t w, uint8_t h, SPIClass *spi,
                                   int8_t rst_pin, int8_t cs_pin,
                                   uint32_t bitrate)
...
/*!
    @brief  Destructor for GP1294VFD object.
*/
GP1294VFD::~GP1294VFD(void) {
  if (buffer) {
    free(buffer);
    buffer = NULL;
  }
...
inline void GP1294VFD::SPIwrite(uint8_t d) {
  if (spi) {
    (void)spi->transfer(d);
  } else {
...
// Issue single command to GP1294VFD, using I2C or hard/soft SPI as needed.
// Because command calls are often grouped, SPI transaction and selection
// must be started/ended in calling function for efficiency.
// This is a private function, not exposed (see ssd1306_command() instead).
void GP1294VFD::GP1294_command(uint8_t c) {
    SPIwrite(c);
}
...
*/
bool GP1294VFD::begin(bool reset,
                             bool periphBegin) {
  ...
  // Reset GP1294 if requested and reset pin specified in constructor
  if (reset && (rstPin >= 0)) {
  ...
  // software reset
  TRANSACTION_START
  GP1294_command(GP1294VFD_SOFTWARERESET);
  TRANSACTION_END
  ...
  //VFD mode setting
  TRANSACTION_START
  GP1294_command(GP1294VFD_SETVFDMODE);
  GP1294_command(0x01);
  GP1294_command(0x1F);
  GP1294_command(0x00);  
  GP1294_command(0xFF);
  ...
  TRANSACTION_START
  GP1294_command(GP1294VFD_SETDIMMING);
  //GP1294_command(0x03);
  //GP1294_command(0xFF);
  GP1294_command(0x01);
  GP1294_command(0x2C);
  ...
  //Clear GRAM
  TRANSACTION_START
  GP1294_command(GP1294VFD_CLEARGRAM);
  TRANSACTION_END
  ...
  TRANSACTION_START
  GP1294_command(GP1294VFD_DISPLAYPOSITIONOFFSET);
  GP1294_command(0x00);
  GP1294_command(0x00);
  TRANSACTION_END  
  ...
  TRANSACTION_START
  GP1294_command(GP1294VFD_SETDISPLAYMODE);
  GP1294_command(0x00);
  TRANSACTION_END 
...
/*!
    ...
    @param  color
            Pixel color, one of: GP1294VFD_BLACK, GP1294VFD_WHITE or GP1294VFD_INVERT.
    @return None (void).
    @note   Changes buffer contents only, no immediate effect on display.
            Follow up with a call to display(), or with other graphics
            commands as needed by one's own application.
*/
void GP1294VFD::drawPixel(int16_t x, int16_t y, uint16_t color) {
  if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    ...
    switch (color) {
    case GP1294VFD_WHITE:
      buffer[x + (y / 8) * WIDTH] |= (1 << (y & 7));
      break;
    case GP1294VFD_BLACK:
      buffer[x + (y / 8) * WIDTH] &= ~(1 << (y & 7));
      break;
    case GP1294VFD_INVERSE:
      buffer[x + (y / 8) * WIDTH] ^= (1 << (y & 7));
      break;
    }
  }
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void GP1294VFD::clearDisplay(void) {
...
/*!
    ...
    @param  color
            Line color, one of: GP1294VFD_BLACK, GP1294VFD_WHITE or GP1294VFD_INVERT.
    @return None (void).
    @note   Changes buffer contents only, no immediate effect on display.
            Follow up with a call to display(), or with other graphics
            commands as needed by one's own application.
*/
void GP1294VFD::drawFastHLine(int16_t x, int16_t y, int16_t w,
                                     uint16_t color) {
  bool bSwap = false;
  switch (rotation) {
  case 1:
...
void GP1294VFD::drawFastHLineInternal(int16_t x, int16_t y, int16_t w,
                                             uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void GP1294VFD::drawFastVLine(int16_t x, int16_t y, int16_t h,
                                     uint16_t color) {
  bool bSwap = false;
  switch (rotation) {
  case 1:
...
void GP1294VFD::drawFastVLineInternal(int16_t x, int16_t __y,
      ...
      if (h >= mod) { // More to go?
        ...
        if (h >= 8) {
          if (color == GP1294VFD_INVERSE) {
            ...
            uint8_t val = (color != GP1294VFD_BLACK) ? 255 : 0;
            do {
              *pBuf = val;   // Set byte
              pBuf += WIDTH; // Advance pointer 8 rows
              h -= 8;        // Subtract 8 rows from height
...
/*!
    ...
    @param  y
            Row of display -- 0 at top to (screen height -1) at bottom.
    @return true if pixel is set (usually GP1294VFD_WHITE, unless display invert
   mode is enabled), false if clear (GP1294VFD_BLACK).
    @note   Reads from buffer contents; may not reflect current contents of
            screen if display() has not been called.
*/
bool GP1294VFD::getPixel(int16_t x, int16_t y) {
  if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch (getRotation()) {
    case 1:
...
*/
uint8_t *GP1294VFD::getBuffer(void) { return buffer; }

// REFRESH DISPLAY ---------------------------------------------------------

/*!
    @brief  Push data currently in RAM to GP1294VFD display.
    @return None (void).
    @note   Drawing operations are not visible until this function is
            called. Call after each graphics command, or after a whole set
            of graphics commands, as best needed by one's own application.
*/
void GP1294VFD::display(void) {
  ...
  // With the limited size of GP1294VFD displays, and the fast bitrate
  ...
  GP1294_command(GP1294VFD_WRITEGRAM);
  GP1294_command(0x00);
  GP1294_command(y_offs*8);
 //   GP1294_command(0x00);
  GP1294_command(0x07);  
...
/*!
    ...
    @note   This has an immediate effect on the display, no need to call the
            ...
            enabled, drawing GP1294VFD_BLACK (value 0) pixels will actually draw
   white, GP1294VFD_WHITE (value 1) will draw black.
*/
void GP1294VFD::invertDisplay(bool i) {
  TRANSACTION_START
  GP1294_command(GP1294VFD_SETDISPLAYMODE);
  GP1294_command(i ? GP1294VFD_INVERTEDDISPLAY : GP1294VFD_NORMALDISPLAY);
  TRANSACTION_END
}
...
*/
void GP1294VFD::dim(uint16_t dim) {
  ...
  TRANSACTION_START
  GP1294_command(GP1294VFD_SETDIMMING);
  GP1294_command(dim_h);
  GP1294_command(dim_l);
  TRANSACTION_END
}
